...
(() => {
    ...
    document.addEventListener('click', (e) => {
        if (e.target.classList.contains('link-item')) {
            let hash = e.target.hash || ''; // Get the hash (or empty string if none)

            // Handle the Home link (index.html) without refresh
            if (e.target.getAttribute('href') === 'index.html' || hash === '#home') {
                e.preventDefault();
                hash = '#home'; // Treat as a link to the home section
            }

            if (hash !== '') {
                e.preventDefault();
                changeSection(hash); // Call function to change section
                window.location.hash = hash; // Set the hash in the URL
                localStorage.setItem('currentSection', hash); // Store section in localStorage
    ...
    function changeSection(hash) {
        ...
        // Check if the active and target sections exist before modifying them
...
// =============== SCROLL TO STORED SECTION OR HASH ON PAGE LOAD ==============
function scrollToStoredSection() {
    const storedSection = localStorage.getItem('currentSection') || window.location.hash;
    if (storedSection) {
        const targetSection = document.querySelector(storedSection);
        if (targetSection) {
            // Hide current active section and show the stored one
            const activeSection = document.querySelector('section.active');
            if (activeSection) {
                activeSection.classList.add('hide');
                activeSection.classList.remove('active');
...
// =============== ENSURE SCROLL AFTER PAGE RELOAD ==============
document.addEventListener('DOMContentLoaded', () => {
    scrollToStoredSection(); // Scroll to stored section on page load
});
